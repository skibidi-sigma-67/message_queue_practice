...
inline void Producer(BaseQueue* target_queue, int thread_id,
  ...
  double mean_delay =
      std::max<double>(1.0, static_cast<double>(base_delay.count()));
  ...
  while (!stop_signal.load(std::memory_order_acquire)) {
    ...
    double latency_microseconds =
        MeasureExecutionTime([&]() { target_queue->Push(generated_message); });

    latencies.push_back(latency_microseconds);

    if constexpr (UseExponentialDelay) {
      long long random_delay_count = static_cast<long long>(
          exponential_distribution(random_number_generator));
...
inline void Consumer(BaseQueue* target_queue, int thread_id,
                     std::chrono::microseconds base_delay,
                     std::atomic<bool>& start_signal,
                     std::atomic<bool>& stop_signal,
                     std::vector<double>& latencies,
  ...
  while (!stop_signal.load(std::memory_order_acquire)) {
    auto [received_message, latency_microseconds] =
        MeasureExecutionTime([&]() { return target_queue->TryPop(); });
